...
export function PdfComparePage() {
  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      showToast(t('pdfCompare.success.complete'), 'success');
    } catch (err) {
      console.error(err);
      showToast(t('pdfCompare.errors.failed'), 'error');
    } finally {
      setComparing(false);
    }
  };
      ...
      <div className="bg-slate-50 dark:bg-[#020617] py-12 px-4 min-h-screen">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">{t('pdfCompare.results.title')}</h1>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-100 dark:border-slate-800 text-center">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">{t('pdfCompare.results.similarity')}</p>
              <p className={`text-3xl font-black ${result.similarity > 80 ? 'text-emerald-500' : result.similarity > 50 ? 'text-amber-500' : 'text-red-500'}`}>
                {result.similarity}%
              </p>
            </div>
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-100 dark:border-slate-800 text-center">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">{t('pdfCompare.results.added')}</p>
              <p className="text-3xl font-black text-emerald-500">+{result.addedCount}</p>
            </div>
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-100 dark:border-slate-800 text-center">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">{t('pdfCompare.results.removed')}</p>
              <p className="text-3xl font-black text-red-500">-{result.removedCount}</p>
            </div>
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-100 dark:border-slate-800 text-center">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">{t('pdfCompare.results.unchanged')}</p>
            ...
            <div className="flex items-center gap-2 px-6 py-4 border-b border-slate-100 dark:border-slate-800">
              {(['all', 'added', 'removed'] as const).map(f => (
                <button key={f} onClick={() => setShowFilter(f)}
                  ...
                  {f === 'all' ? t('pdfCompare.results.all') : f === 'added' ? `+ ${t('pdfCompare.results.added')} (${result.addedCount})` : `- ${t('pdfCompare.results.removed')} (${result.removedCount})`}
                </button>
              ))}
            ...
            <div className="max-h-[60vh] overflow-y-auto">
              {filteredDiffs.length === 0 ? (
                <div className="p-12 text-center text-slate-400">
                  ...
                  <p className="font-bold">{t('pdfCompare.results.noDiffs')}</p>
                </div>
              ) : (
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-purple-500/10 text-purple-600 rounded-full mb-6">
            ...
            <span className="text-[10px] font-black uppercase tracking-[2px]">{t('pdfCompare.badge')}</span>
          </div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium text-lg max-w-2xl mx-auto">
            {t('pdfCompare.subtitle')}
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          {[
            { file: file1, setFile: setFile1, label: t('pdfCompare.docA') },
            { file: file2, setFile: setFile2, label: t('pdfCompare.docB') },
              ...
              {file ? (
                ...
                <label className="cursor-pointer block">
                  ...
                  <div className="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl p-10 hover:border-indigo-500/50 transition-colors">
                    ...
                    <p className="text-sm font-bold text-slate-400">{t('pdfCompare.uploadPrompt')}</p>
                  </div>
                </label>
              )}
            </div>
        ...
        <div className="text-center">
          ...
          >
            {comparing ? (
              <><Loader2 className="w-4 h-4 animate-spin" /> {t('pdfCompare.comparing')}</>
            ) : (
              <><GitCompareArrows className="w-4 h-4" /> {t('pdfCompare.compareDocs')}</>
            )}
          </button>
        </div>
      </div>
